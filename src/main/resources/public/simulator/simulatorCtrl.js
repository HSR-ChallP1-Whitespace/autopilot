...
angular
		...
		.controller(
				'simulatorCtrl',
				function($scope, $routeParams, Simulator, ngstomp) {
					...
					$scope.availableDesigns = [ $scope.selectedDesign,
							"Berlin", "Oerlikon", "Hollywood", "Budapest" ];
					...
					$scope.displaySpecifications = [ {
						...
						description : 'Magnetic X',
						coord : {
							vector : 'm',
							axis : 0
						}
					}, {
						description : 'Magnetic Y',
						coord : {
							vector : 'm',
							axis : 1
						}
					}, {
						description : 'Magnetic Z',
						coord : {
							vector : 'm',
							axis : 2
						}
					...
					$scope.sizeOptions = [ $scope.sizeOption, {
						description : "800x600",
						width : 800,
						height : 600
					}, {
					...
					$scope.selectCanvas = function() {
						console.log("choosing canvas "
								+ $scope.sizeOption.description);
					...
					$scope.connect = function() {
						...
						var REST_API_URL = '';
						$scope.ngStompClient = ngstomp(REST_API_URL
								+ '/messages');
						$scope.ngStompClient
								.connect(
										{},
										function(client, frame) {
											...
											/*
											 * $scope.ngStompClient.subscribe('/topic/echo',
											...
											$scope.ngStompClient
													.subscribe(
															'/topic/simulator/clock',
															function(message) {
																var msg = JSON
																		.parse(message.body).timestamp;
																$scope.recentTime = msg;
															});

											$scope.ngStompClient
													.subscribe(
															'/topic/simulator/news',
															function(message) {
					...
					$scope.echo = function() {
						$scope.ngStompClient.send('/stomp/messages', {}, JSON
								.stringify({
									'message' : 'echo: '
					...
					var adjustScale = function() {
						...
						var px_cm_x = ($scope.canvasWidth - 2 * $scope.padding)
								/ $scope.boundaryWidth;
						var px_cm_y = ($scope.canvasHeight - 2 * $scope.padding)
								/ $scope.boundaryHeight;
						$scope.scaleFactor = Math.min(px_cm_x, px_cm_y);

						$scope.baseAnchor = {
							angle : $scope.baseAnchor.angle,
							x : $scope.padding + $scope.originalAnchorX
									* $scope.scaleFactor,
							y : $scope.padding + $scope.originalAnchorY
									* $scope.scaleFactor
						};
						drawOnCanvas();
					};
					...
					var drawOnCanvas = function() {
						...
						tracks.forEach(function(sector) {
							...
							if (angular.isUndefined(sector.radius)) {
								...
								anchor = straight(ctx, anchor, scale
										* sector.length);
							} else {
								anchor = curve(ctx, anchor, scale
										* sector.radius,
										Math.abs(sector.angle),
										sector.orientation === "LEFT");
							}
						});
					...
					var drawFinal = function(ctx, anchor) {
						...
						ctx.beginPath();
						ctx.rect(anchor.x - (width / 2), anchor.y
					...
					/**
					 * Intended for value range [0.1 - 10000] to logarithmically
					 * map it down to [0-500] by preserving accuracy on small
					 * values
					 * 
					 * @param x
					 * @returns {number}
					...
					var drawVelocity = function() {
						if (angular.isDefined($scope.recentNews.position)) {
							...
							var width = canvas.width
									* ($scope.recentNews.velocity / $scope.MAX_SPEED);
					...
					var drawPower = function() {
						if (angular.isDefined($scope.recentNews.position)) {
							...
							ctx.fillStyle = "#A02000";
							var width = canvas.width
									* ($scope.recentNews.currentPower / 260);
							ctx.rect(0, 0, width, canvas.height);
							ctx.fill();
							ctx.closePath();
						}
					...
					var drawPosition = function(ctx) {
						...
						if (angular.isDefined($scope.recentNews.position)) {
							...
							var r = $scope.width / 2;
							ctx.beginPath();
							var posx = $scope.baseAnchor.x
									+ $scope.recentNews.position.posX
									* $scope.scaleFactor;
							var posy = $scope.baseAnchor.y
									+ $scope.recentNews.position.posY
									* $scope.scaleFactor;
					 ...
					 *            the canvas context
					 * @param anchor
					 *            the anchor = position and direction of the
					 *            section to be created
					 * @param radius
					 *            the curvature radius of the curved section
					 * @param angle
					 *            the curvature angle of the curved section
					 * @param ccw
					 *            indicator for counter-clock-wise curvature.
					...
					var curve = function(ctx, anchor, radius, angle, ccw) {
						...
						ctx.lineTo(center.x + inner * Math.cos(ang1), center.y
								+ inner * Math.sin(ang1));
					...
					var straight = function(ctx, anchor, length) {
						...
						var p1 = {
							x : anchor.x - sin * width / 2.0,
							y : anchor.y - cos * width / 2.0
						};
						var p2 = {
							x : p1.x + cos * length,
							y : p1.y - sin * length
					...
					$scope.saveParams = function() {
						Simulator.saveParams($scope.gaugeParams, {
							controller : "params"
						}, function() {
