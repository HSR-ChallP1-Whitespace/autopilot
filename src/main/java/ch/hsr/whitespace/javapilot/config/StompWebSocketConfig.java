...
public class StompWebSocketConfig extends AbstractWebSocketMessageBrokerConfigurer {
	...
	public void configureMessageBroker(MessageBrokerRegistry config) {
		...
		/**
		 * The given channel-prefixes are used to filter out incoming SEND
		 * commands. Only those matching this filter will be handed over to the
		 * Controllers for further processing.
		 *
		 * A client should therefore send a message to a destination channel
		 * like:
		 *
		 * channel: /app/echo
		 *
	...
	public void registerStompEndpoints(StompEndpointRegistry registry) {
		...
		// Register a pure WebSocket endpoint
		// (ws://myserver.com/ws/rest/messages)
