package ch.hsr.whitespace.javapilot.akka.experimental;
...
/**
 * this logic node increases the power level by 10 units per 0.5 second until it
 * receives a penalty then reduces by ten units.
 */
public class PowerUpUntilPenalty extends UntypedActor {
	...
	/**
	 * @param pilotActor
	 *            The central pilot actor
	 * @param duration
	 *            the period between two increases
	 * @return the actor props
	 */
	public static Props props(ActorRef pilotActor, int duration) {
		return Props.create(PowerUpUntilPenalty.class, () -> new PowerUpUntilPenalty(pilotActor, duration));
	}
	...
	/**
	 * Strategy: increase quickly when standing still to overcome haptic
	 * friction then increase slowly. Probing phase will be ended by the first
	 * penalty
	 * 
	 * @param message
	 *            the sensor event coming in
	 */
