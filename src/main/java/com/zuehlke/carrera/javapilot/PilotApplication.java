...
import com.zuehlke.carrera.connection.ConnectionFactoryFromPilots;
import com.zuehlke.carrera.connection.ConnectionFactoryTowardsPilots;
import com.zuehlke.carrera.connection.Protocol;
import com.zuehlke.carrera.connection.RabbitConnectionFactoryFromPilots;
import com.zuehlke.carrera.connection.RabbitConnectionFactoryTowardsPilots;
...
@EnableConfigurationProperties({ SimulatorProperties.class }) // loaded from
...
public class PilotApplication implements CommandLineRunner {
	...
	private void connectPilotWithRabbit(ActorRef pilot) {
		...
		ConnectionFactoryFromPilots factory = new RabbitConnectionFactoryFromPilots(pilotApi, settings.getName(), settings.getAccessCode(), settings.getRabbitUrl());

		PilotToRelayConnection pilotConnection = factory.create((start) -> pilot.tell(start, ActorRef.noSender()), (stop) -> pilot.tell(stop, ActorRef.noSender()),
				(sensor) -> pilot.tell(sensor, ActorRef.noSender()), (velo) -> pilot.tell(velo, ActorRef.noSender()), (penalty) -> pilot.tell(penalty, ActorRef.noSender()),
	...
	private void connectSimulatorWithRabbit(RaceTrackSimulatorSystem system) {
		...
		client.connect(settings.getRabbitUrl());
		Serializer serializer = new JacksonSerializer();
		TowardsPilotApi towardsPilotApi = new SimulatorTowardsPilotApiImpl(client, new PilotToRelayChannelNames(settings.getName()), serializer);
